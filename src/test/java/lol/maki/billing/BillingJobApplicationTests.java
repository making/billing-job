...
class BillingJobApplicationTests {
	...
	@Test
	void testJobResults() {
		List<Bill> billStatements = this.jdbcTemplate.query(
				"select id, first_name, last_name, minutes, data_usage, bill_amount FROM bill_statements ORDER BY id",
				(rs, rowNum) -> new Bill(rs.getLong("id"), rs.getString("first_name"), rs.getString("last_name"),
						rs.getLong("data_usage"), rs.getLong("minutes"), rs.getBigDecimal("bill_amount")));
